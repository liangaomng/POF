...
def plot_test_performance(folder,epoch,**kwargs):
    ...
    ax[1].plot(range(num_points),g_SSIM,label="Global SSIM",linestyle='-',
               linewidth=3,marker="o")
    ax[1].plot(range(num_points),l_SSIM,label="Global SSIM",linestyle='--',
               linewidth=3,marker="s")
    ...
    ax[3].plot(range(num_points),test_losses, label="Test loss",linewidth=2,
             linestyle='--')
    ax[3].legend()
    ax[3].set_yscale('log')
